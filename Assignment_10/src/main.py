...
from src.errors.errors import BookError, ValidationError, ClientError, RentalError
...
from src.initializer.initializer import generate_books, generate_clients, generate_rentals
...
if __name__ == "__main__":
    ...
    book_service = BookService(book_repo, book_validator, undo_service)
    ...
    client_service._ClientService__rental_service = rental_service
    book_service._BookService__rental_service = rental_service
    ...
    if not rentals:
        ...
        for r in rentals:
            try:
                rental_service.rent_book(r.book_id, r.client_id, days=r.days)
            except (RentalError, ValidationError) as e:
                print(f"Error adding client {r.rental_id}: {e}")

    menu = Menu(book_service, client_service, rental_service, undo_service)
    menu.menu()
